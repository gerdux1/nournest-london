...
export function Planner() {
  ...
  function pickProperty(slug: string, areaSlug: string) {
  ...
  async function reveal() {
    ...
    try {
      const res = await fetch("/api/plan", {
        ...
        body: JSON.stringify({ who, size, nights, area, propertySlug, arrival, interests, pace, email: email.trim(), name: name.trim() }),
      });
      const data = (await res.json()) as PlanResult;
      setResult(data);
      setStep(STEPS);
    } catch {
      setResult({ headline: "Your London plan", note: "We'll email your full plan shortly.", days: [], areaSlug: area });
      setStep(STEPS);
    } finally {
      setLoading(false);
    }
  ...
  function restart() {
    ...
    setPropertySlug(null); setInterests([]); setPace(null); setEmail(""); setName("");
    setResult(null); setEmailErr(false);
  }
        ...
        <div className="np-step">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-14 text-center">
              ...
              <p className="mt-2 text-[#555]">Mapping your days and the smartest way around…</p>
            ...
            <>
              ...
              <p className="mt-2 text-[#555]">Pop your email in and your personalised London itinerary appears instantly.</p>
              ...
              <div className="mt-6 flex items-center justify-between">
                <button onClick={() => setStep(5)} className="text-sm text-[#555] underline">Back</button>
                <button onClick={reveal} className="rounded-full bg-[#FFDE59] px-6 py-3 font-semibold tracking-wide text-[#385B4F] transition hover:bg-[#f5d240]">
                  REVEAL MY PLAN ✨
                </button>
              </div>
            </>
      ...
      {step === 7 && result && (
        <div className="np-step">
          ...
          <div className="mt-3 rounded-xl bg-[#F3FADC] px-4 py-3 text-sm text-[#385B4F]">
            ✉️ Full plan on its way to {email || "your inbox"}.
          </div>
